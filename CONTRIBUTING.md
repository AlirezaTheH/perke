...
- **Identify the Bug**: Link to the issue describing the bug that you're fixing. 
  ...
  > Note: In some cases, one person's "bug" is another person's "feature." If 
  > the pull request does not address an existing issue with the "bug" label, 
  > the maintainers have the final say on whether the current behavior is a 
  > bug.
...
- All codes should be documented, to the same 
  ...
  as NumPy and SciPy. See [documentation](#documentation).
